...
W1_mag = list(zip(W1_mag, mjd_date_W1))
W2_mag = filtered_rows_W2.iloc[:, 9]
W2_mag = list(zip(W2_mag, mjd_date_W2))
...
if len(W1_mag) == len(W2_mag):
    for i in range(len(W1_mag)):
        W1_list = []
        W2_list = []
        colour.append(W1_mag[i][0] - W2_mag[i][0])
        mjd_date_colour.append(W1_mag[i][1])
        #doing averaging now
        if i == 0:
            pass
        else:
            if W1_mag[i][1] - W1_mag[i-1][1] < 100: #collecting the dates that are bunched together and averaging
                W1_list.append(W1_mag[i-1][0])
                W2_list.append(W2_mag[i-1][0])
    ...
    for i in range(len(W2_mag)):
        if W2_mag[i][1] == W1_mag[i][1]: #checking if mjd dates are the same
            print(i)
            colour.append(W1_mag[i][0] - W2_mag[i][0])
            mjd_date_colour.append(W1_mag[i][1])
            print(W2_mag[i][1] - W2_mag[i-1][1])
            if i == 0:
                W1_list.append(W1_mag[i][0])
                W2_list.append(W2_mag[i][0])
                continue
            elif W2_mag[i][1] - W2_mag[i-1][1] < 100:
                print('path')
                W1_list.append(W1_mag[i-1][0])
                W2_list.append(W2_mag[i-1][0])
                continue #need to check if at the end.
    ...
    for i in range(len(W1_mag)):
        if W2_mag[i][1] == W1_mag[i][1]:
            ...
            if i == 0:
                ...
                if W1_mag[i][1] - W1_mag[i-1][1] < 100:
                    W1_list.append(W1_mag[i-1][0])
                    W2_list.append(W2_mag[i-1][0])
                W1_averages.append(np.average(W1_list))
                W2_averages.append(np.average(W2_list))
...
#need average mjd now.

# for i in range(len(mjd_date_W1)):
#     if i == 0:
#         pass
#     else:
#         print(mjd_date_W1[i]-mjd_date_W1[i-1])
...
plt.scatter(mjd_date_colour, colour, color = 'red', label = r'Colour (W1 Flux - W2 Flux)')
plt.xlabel('Date / mjd date')
plt.ylabel('Magnitude')
plt.title('Colour vs time (ph_qual > B)')
plt.legend(loc = 'upper right')
# plt.show()